...
import { SpeakerFormFields } from "@/utils";
...
const SpeakerForm = () => {
  ...
  const firstSix = SpeakerFormFields.slice(0, 6); // Gets the first 6 items
  const lastTwo = SpeakerFormFields.slice(-2); // Gets the last 2 items
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const speakertitle = formData.get("affiliation") as string | null;
      ...
      <form onSubmit={handleSubmit} ref={formRef} className={styles.form}>
        <div className={styles.form__top}>
          <div className={styles.form__boxing}>
            ...
            <div className={styles.form__forms}>
              {lastTwo.map((data, i) => (
                <FormInput data={data} key={`lt${i}`} isSubmitting={isSubmitting} />
              ))}
            </div>
          </div>
