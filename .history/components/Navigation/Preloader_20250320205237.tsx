  ...
  useEffect(() => {
    if (finished) {
      if (pathname === "/") {
        ...
        // For other pages, we don't need the animation to hide the preloader,
        // so we immediately trigger the completion.
        setAnimation(true);
      }
    }
  }, [finished, pathname, setAnimation]);
      ...
      {pathname === "/" ? (
        ...
        <div className={styles.preload__other}>
          <div className={styles.otext}>
            <AnimatedPhrase phrase={pageName} />
          </div>
        </div>
      )}
    </div>
