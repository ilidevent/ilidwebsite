...
const Button = ({ buttonInfo, fill, onClick }: ButtonProps) => {
  ...
  if (buttonInfo.link) {
    return fill ? (
      <Link className={styles.button__background} href={buttonInfo.link} target={buttonInfo.}>
        {buttonInfo.name}
      </Link>
    ) : (
      <Link className={styles.button__stroke} href={buttonInfo.link} target={buttonInfo.}>
        {buttonInfo.name}
      </Link>
    );
  }
