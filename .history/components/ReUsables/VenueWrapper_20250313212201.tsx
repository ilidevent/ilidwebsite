...
const VenueWrapper = () => {
  ...
  return (
    <div className={styles.venue__wrapper}>
        ...
        {venueImages.map((data, i) => (
          <div
            className={`${styles.venueimage} ${
              activeImage === i ? styles.active__image : ""
            }`}
            key={data.id}
          >
      ...
      <div className={styles.venue__right}>
        {venueImages.map((data, i) => (
          <div className={`${styles.ivenue__wrapper} V`}>
            <div
              className={`${styles.venueimagetwo}`}
              key={data.id}
              onClick={() => setActiveImage(i)}
            >
              <Image fill quality={100} alt={data.alt} src={data.image} />
            </div>
            <div className={styles.venue__overlay}></div>
          </div>
        ))}
      </div>
    </div>
